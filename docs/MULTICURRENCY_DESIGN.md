...
The `Entry` entity has been removed from the data model. Operations now belong directly to a transaction via `transactionId`. A transaction contains from 2 to 1000 active operations; balance is checked at the transaction level by summing `value`.
...
- From 2 to 1000 active operations per transaction
